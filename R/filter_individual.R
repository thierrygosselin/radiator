...
filter_individual <- function(
  ...
  if (interactive.filter) {
    ...
    path.folder <- stringi::stri_join(getwd(),"/", "filter_individual_", file.date, sep = "")
  ...
  if (interactive.filter) {
    ...
    if (line.graph == "y") {
      ...
      threshold.helper <- dplyr::bind_rows(threshold.helper.pop, mean.pop, threshold.helper.overall) %>% 
        dplyr::mutate(
      ...
      # save
      ggsave(stringi::stri_join(path.folder, "/plot.ind.threshold.pdf"), width = pop.number*4, height = 10, dpi = 600, units = "cm", useDingbats = F)
      ggsave(stringi::stri_join(path.folder, "/plot.ind.threshold.png"), width = pop.number*4, height = 10, dpi = 300, units = "cm")
      message(stringi::stri_join("2 versions (pdf and png) of the line graph of individual threshold on marker discovery were saved in this directory: \n", path.folder))
      ...
      readr::write_tsv(x = ind.threshold.helper.table, path = stringi::stri_join(path.folder, "/", "ind.threshold.helper.table.tsv"))
      message(stringi::stri_join("ind.threshold.helper.table was written in this directory: \n", path.folder))
    }
  }
...
Enter the number of problematic population that you allow to deviate from the threshold:")
  ...
  if (data.type == "vcf.file") {
    # markers.chrom.locus.pos <- input %>% dplyr::distinct(MARKERS, LOCUS)
  ...
  readr::write_tsv(whitelist.markers, "whitelist.markers.ind.tsv", append = FALSE, col_names = TRUE)
  ...
  readr::write_tsv(blacklist.markers, "blacklist.markers.ind.tsv", append = FALSE, col_names = TRUE)
  
  # results --------------------------------------------------------------------
  cat("############################### RESULTS ###############################\n")
  message(stringi::stri_join("ind.approach: ", ind.approach))
  message(stringi::stri_join("ind.threshold: ", ">= ", ind.threshold, " ", threshold.id))
  message(stringi::stri_join("prob.pop.threshold: ", prob.pop.threshold))
  if (data.type != "vcf.file" & data.type != "haplo.file") {
    message(stringi::stri_join("The number of markers removed by the Individual filter: ", dplyr::n_distinct(input$MARKERS) - dplyr::n_distinct(filter$MARKERS)))
    ...
    message(stringi::stri_join("The number of markers/locus removed by the Individual filter: ", dplyr::n_distinct(input$MARKERS) - dplyr::n_distinct(filter$MARKERS)))
    message("The number of markers before -> after the Individual filter")
