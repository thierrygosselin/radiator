...
It's not really complicated, to see if you're going to like radiator there just 
2 functions to try.

**1. Prepare a strata file**
* It's a tab separated file, e.g. `radiator.strata.tsv`
* A minimum of 2 columns: `INDIVIDUALS` and `STRATA` is required. `STRATA` could
be your populations or sampling sites names or any grouping you want.
...
It's my ONE FUNCTION TO RULE THEM ALL. There's obviouly more to this. The function 
is made of modules (see below) that user's can access separately or in combination. 
