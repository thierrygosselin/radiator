...
* the current version is macOS Big Sur 11.0.1
...
#In Terminal
cd Downloads
sudo tar -xzvf clang+llvm-11.0.0-x86_64-apple-darwin.tar.xz -C/usr/local --strip-components=1
```


```{bash eval=FALSE}
sudo rm clang+llvm-11.0.0-x86_64-apple-darwin.tar.xz
clang -v
```
...
To install R v4.0.3 **"Bunny-Wunnies Freak Out"** released on 2020-10-10 download the
installer and follow the instructions
```{bash eval=FALSE}
# for macOS
cd Downloads
curl -O https://cran.r-project.org/bin/macosx/R-4.0.3.pkg
```
...
**With macOS** version 11.0.1 (Big Sur): follow the `.Makevars` [configuration](#Makevars_config)
