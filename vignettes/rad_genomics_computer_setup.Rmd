...
## Error: `dyn.load` and `library not loaded` when installing `vcfppR` {#vcfppR}

```{bash, eval=FALSE}
Library not loaded: @rpath/libunwind.1.dylib  
Referenced from: /usr/local/lib/libc++.1.0.dylib  
Reason: tried: '/usr/local/lib/libunwind.1.dylib' (no such file)
```

**Diagnosis**

You’re on macOS (ARM64, Apple Silicon) and using a custom LLVM/Clang install `/usr/local/bin/clang++` instead of the system compiler `/usr/bin/clang++`.
This version of libc++ `/usr/local/lib/libc++.1.0.dylib` expects libunwind to be installed at `/usr/local/lib/libunwind.1.dylib`, but it isn’t found.
macOS’ system libunwind is in `/usr/lib`, which user-installed clang doesn’t 
automatically link against.

Check the ownership and if it’s not symlinked to `/usr/lib/libc++.1.dylib`, consider removing or renaming it:
```{bash, eval=FALSE}
ls -l /usr/local/lib/libc++.*
```

```{bash, eval=FALSE}
sudo mv /usr/local/lib/libc++.1.dylib /usr/local/lib/libc++.1.dylib.bak
```
